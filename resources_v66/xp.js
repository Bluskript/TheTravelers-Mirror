    ...
    apply: function (json) {
        ...
        if (json.level !== undefined) {
            if (json.level > XP.level) {
                if (json.level <= 3) {
                    ENGINE.log("leveled up! you are now level <b>" + (json.level + 1) + "</b> and you've gained one skill point to use to upgrade your health, damage, or stamina.");
                } else {
                    ENGINE.log("leveled up! you are now level <b>" + (json.level + 1) + "</b>.");
                }
