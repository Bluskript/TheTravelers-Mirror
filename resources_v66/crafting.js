var CRAFTING = {
    ...
    setHtml: function (json) {
        ...
        for (let i = 0; i < Object.keys(json).length; i++) {
            ...
            for (let j = 0; j < arr.length; j++) {
                ...
                if (SUPPLIES.sortStyle === "icon") {
                    text.push({
                        "text": "<div class='supplies-box-icon' style='opacity:0.5'><div class='supplies-icon-symbol' " + (icon.length >= 3 ? "style='font-size: 18px;'" : "") + ">" + icon + "</div><div id='craft-" + name + "' onclick='CRAFTING.open(this.id)' class='supplies-icon-hover no-hover'></div></div>",
                        "name": name
                    });
                } else {
                    text.push({
        ...
        if (!ENGINE.isFirstConnect && hasNewItems) {
            ENGINE.log("you have new items available to craft.");
        }

        CRAFTING.scrollEl.innerHTML = html;
    ...
    open: function (id) {
        ...
        if (SUPPLIES.current[obj.name] !== undefined) CRAFTING.craftDescEl.innerHTML = "<span style='font-weight: bold;display: block;margin: -5px 0 10px;'>you have " + SUPPLIES.current[obj.name].count + "</span>";
        else CRAFTING.craftDescEl.innerHTML = "";
        CRAFTING.craftDescEl.innerHTML += obj.desc;
